...
void get_data(t_data *data, int elapsed_time)
{
    if (data->mode & MODE_SIMPLE) {
        u16 humidity;
        HTS221_measure(&(data->cur_temp), &humidity);
...
int get_send_timer(u8 data->mode)
{
    if (data->mode & MODE_TEST)
        return (SEND_MSG_TEST);
...
int get_sleep_timer(u8 data->mode)
{
    if (data->mode & MODE_TEST)
        return (CALC_TEST);
    else if (data->mode & MODE_LOWBATT)
...
int main()
{
    ...
    elapsed_time = get_send_timer(data.mode);
    
    while (TRUE)
    {
        //DEFINIG NEXT WAKE UP TIME
        SENSIT_API_set_rtc_alarm(get_sleep_timer(data.mode));
        ...
        {
            elapsed_time -= get_sleep_timer(data.mode);
        ...
        {
            elapsed_time = get_send_timer(data.mode);
            data.mode = set_mode(data.mode, BUTTON_handler());
